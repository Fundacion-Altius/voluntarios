...
#### Scenario: User table schema in Supabase

- **WHEN** the Supabase migration is applied
- **THEN** a `users` table SHALL exist with columns: `id` (UUID, PK), `email` (text, unique, not null), `name` (text), `role` (text, default 'general'), `created_at` (timestamptz, default now())

#### Scenario: User table schema in MariaDB

- **WHEN** the MariaDB migration is applied
- **THEN** a `users` table SHALL exist with columns: `id` (VARCHAR 36, PK), `email` (VARCHAR 255, unique, not null), `name` (VARCHAR 255), `role` (VARCHAR 20, default 'general'), `created_at` (TIMESTAMP, default CURRENT_TIMESTAMP)
