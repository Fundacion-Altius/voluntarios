...
- [x] 5.1 Remove `page.route()` mocks from `voluntarios-front/e2e/survey-flow.spec.ts`
- [x] 5.2 Rewrite survey-loads-and-displays test to use real `/api/questions` endpoint
- [x] 5.3 Rewrite successful-submission test to POST to real backend without CSRF token
- [x] 5.4 Add anonymous-access test that verifies POST without auth succeeds (no 403)
- [x] 5.5 Remove or adapt error-handling test that relied on route overrides
...
- [x] 6.5 Run `pnpm run test:e2e` in `voluntarios-front/` with backend running in dev mode
